...
char *usage = "\nUsage: ./kmercount_with_localmax [background filename] [signal filename] [read length (max 64)] [min_kmerlength] [max_kmerlength] [base frequency cutoff for selecting a longer kmer as local maxima (default 0.35)] [gaps counted? counts center gap only by default, 'a'=count all gaps, 'n'=count only ungapped kmers]\nReads must be on individual lines with bases in all caps";
